...
for (const stmt of [
  ...
  // COPPA verifiable parental consent ("email-plus"): the parent must click an emailed link
...
// One-time grandfather of accounts created BEFORE the email-verification requirement shipped
// (2026-07-30): they consented under the prior flow (and paid accounts carry card-based consent),
// so mark them verified rather than locking existing families out. Guarded by the creation-date
// cutoff so NEW signups must verify; idempotent (re-running only re-sets already-1 rows to 1).
try {
