...
router.post('/auth/signup', loginLimiter, (req, res) => {
  ...
  try {
    ...
    // COPPA email-plus: new accounts must confirm their email before adding a child. Issue a
    // single-use verify token + email the link; the account is logged in but /kids is gated.
    const p0 = db.prepare('SELECT * FROM parents WHERE id=?').get(id);
    try {
      const vtoken = require('crypto').randomBytes(24).toString('hex');
      db.prepare('UPDATE parents SET verify_token=? WHERE id=?').run(vtoken, id);
      const origin = process.env.APP_ORIGIN || (req.protocol + '://' + req.get('host'));
      mailer.sendEmailVerification(p0, `${origin}/api/auth/verify-email?token=${vtoken}`);
    } catch (e) { /* email must never break signup */ }
    // Fire-and-forget welcome email (never blocks or fails the signup itself)
    mailer.sendWelcomeTrial(p0);
...
// ---------- COPPA email-plus: verifiable parental consent via email confirmation ----------
router.get('/auth/verify-email', (req, res) => {
  const token = String((req.query || {}).token || '');
  const p = token ? db.prepare('SELECT * FROM parents WHERE verify_token=?').get(token) : null;
  if (!p) return res.redirect('/?verify=invalid#parent');
  if (!p.email_verified) {
    db.prepare("UPDATE parents SET email_verified=1, verified_at=datetime('now'), verify_token=NULL WHERE id=?").run(p.id);
    db.recordConsent({ parentId: p.id, parentEmail: p.email, method: 'email_plus', detail: 'email verification link confirmed (verifiable parental consent)' });
    try { mailer.sendConsentConfirmed(p); } catch (e) {}
  }
  res.redirect('/?verified=1#parent');
});
router.post('/auth/resend-verification', loginLimiter, auth.requireParent, (req, res) => {
  if (req.parent.email_verified) return res.json({ ok: true, alreadyVerified: true });
  try {
    const vtoken = crypto.randomBytes(24).toString('hex');
    db.prepare('UPDATE parents SET verify_token=? WHERE id=?').run(vtoken, req.parent.id);
    const origin = process.env.APP_ORIGIN || `${req.protocol}://${req.headers.host}`;
    mailer.sendEmailVerification(req.parent, `${origin}/api/auth/verify-email?token=${vtoken}`);
  } catch (e) { /* never throw from an email path */ }
  res.json({ ok: true });
});
...
router.get('/auth/me', (req, res) => {
  ...
  if (s.kind === 'parent') {
    const p = db.prepare('SELECT id, email, name, sub_status, sub_plan, trial_ends, is_admin, account_type, school_name, email_verified FROM parents WHERE id=?').get(s.ref_id);
...
router.post('/kids', auth.requireParent, (req, res) => {
  // COPPA email-plus gate: no child profile is created until the parent has confirmed their email
  // (verifiable parental consent). Pre-2026-07-30 accounts are grandfathered verified.
  if (!req.parent.email_verified)
    return res.status(403).json({ error: 'Please confirm your email first — we sent you a verification link. This verifies parental consent before any learner profile is created.', needVerify: true });
...
// Parent view of a child's OPTIONAL Growth Skills course progress + latest self-assessment
// (read-only). Verifies ownership, then returns the growth-course state from game_progress.
router.get('/family/growth/:kidId', auth.requireParent, (req, res) => {
  const kidId = Number(req.params.kidId);
  const kid = db.prepare('SELECT id, name FROM kids WHERE id=? AND parent_id=?').get(kidId, req.parent.id);
  if (!kid) return res.status(404).json({ error: 'Not found' });
  let done = {}, check = null;
  try {
    const row = db.prepare("SELECT data FROM game_progress WHERE kid_id=? AND game='growth'").get(kidId);
    if (row && row.data) { const st = JSON.parse(row.data); if (st) { done = st.done || {}; check = st.check || null; } }
  } catch (e) {}
  res.json({ kid: { id: kid.id, name: kid.name }, done, check });
});
...
router.get('/admin/overview', auth.requireAdmin, (req, res) => {
  ...
  try {
    // Exclude internal dev/QA addresses so the card reflects REAL-customer deliverability, not
    // noise from fake test accounts (which the provider rejects and the sweeps no longer touch).
    const nt = "to_email NOT LIKE '%@example.com' AND to_email NOT LIKE '%@example.org' AND to_email NOT LIKE '%@example.net' AND to_email NOT LIKE '%@test.com' AND to_email NOT LIKE '%@t.com' AND to_email NOT LIKE '%@gallop-test.com' AND to_email NOT LIKE '%@gallop.test'";
    emailHealth.byStatus = Object.fromEntries(db.prepare(`SELECT status, COUNT(*) AS n FROM email_log WHERE created_at >= datetime('now','-7 days') AND ${nt} GROUP BY status`).all().map(r => [r.status, r.n]));
    emailHealth.last24 = db.prepare(`SELECT COUNT(*) AS n FROM email_log WHERE created_at >= datetime('now','-1 day') AND ${nt}`).get().n;
    emailHealth.byKind = db.prepare(`SELECT kind, COUNT(*) AS n, SUM(CASE WHEN status='sent' THEN 1 ELSE 0 END) AS sent, SUM(CASE WHEN status='failed' THEN 1 ELSE 0 END) AS failed FROM email_log WHERE created_at >= datetime('now','-30 days') AND ${nt} GROUP BY kind ORDER BY n DESC`).all();
